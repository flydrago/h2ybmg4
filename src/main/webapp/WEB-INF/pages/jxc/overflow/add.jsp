...
<head>
    ...
    <script type="text/javascript">
    	    ...
    	    if(data.length != 0){
    	  	  ...
    	  	  for(var i=0;i<data.length;i++){
    				...
    				checkedGoodsHtml +=   '<tr id="billGoods_tr_'+data[i].ID+'" class="billGoodsDetail">'+
    		  								...
    		  								'<td class="h2y_table_edit_td"><input name="billGoodsOverflowCount" id="billGoodsOverflowCount'+data[i].ID+'" type="text" class="h2y_input_just billGoodsOverflowCount" style="width:60px;"  value="1" onblur="calcBillGoodsAmount('+data[i].ID+')"/></td>'+
    		  								...
    		  								'<td class="h2y_table_edit_td"><input name="billGoodsOverflowAmount" id="billGoodsOverflowAmount'+data[i].ID+'" type="text" class="h2y_input_just billGoodsOverflowAmount" style="width:100px;" value="0.00" disabled="disabled"/></td>'+
    	...
    	function calcBillGoodsAmount(goodsID){
    		var billGoodsOverflowAmount = accMul($("#billGoodsOverflowCount"+goodsID).val(), $("#billGoodsCostSinglePrice"+goodsID).val());
    		$("#billGoodsOverflowAmount"+goodsID).val(billGoodsOverflowAmount);
    		calcBillTotalData();
    	}
    	
    	//计算单据 库存总数  盘点总数  盈亏总数  盈亏总额
    	function calcBillTotalData(){
    		var billOverflowCount = 0;
    		var billOverflowAmount = 0.00;
    		
    		//报溢总数
    		$(".billGoodsOverflowCount").each(function(){
    			billOverflowCount = accAdd(billOverflowCount, this.value);
    		});
    		$("#overflowCount").val(billOverflowCount);
    		
    		
    		//报溢总额
    		$(".billGoodsOverflowAmount").each(function(){
				billOverflowAmount = accAdd(billOverflowAmount, this.value);    			
    		});
    		$("#overflowAmount").val(billOverflowAmount);
    	}
    	...
    	function h2y_save(){
    		...
    		postData.overflowDate = $("#overflowDate").val();
    		...
    		//单据 商品明细数组 组装
    			$(".billGoodsDetail").each(function(){
    				...
    				billGoodsSingleDetail.goodsOverflowCount = $(this).find("input[name='billGoodsOverflowCount']").val();
    		...
    		$.post("jxc/overflow/save.htm",{postData:JSON.stringify(postData)},function(res){
    			var resJson = eval("("+res+")");
    			if(resJson.resultFlag == 1 || resJson.resultFlag == "1"){
    				alert("单据添加成功。");
    				if (top.f_getframe("jxc_overflow_init_htm") != null) {
						top.f_getframe("jxc_overflow_init_htm").f_query();
					}
					top.f_delTab("overflow_add");
    			}else{
    				/* alert("单据添加失败，请联系管理员"); */
...
<body>
	...
	<form name="editform" method="post" action="" id="editform">
		...
		<table class="h2y_table" style="width:98%;">
			...
			<tr>
				<td class="h2y_table_label_td">
					<input type="button" value="选择报溢仓库" class="button" id="choose_storage_btn"/>：
				</td>
			...
			<tr>
				...
				<td class="h2y_table_edit_td2" colspan="3">
					<input name="overflowDate" id="overflowDate" class="h2y_input_datetime" type="input" 
						value="<fmt:formatDate value="${bill.overflowDate}"  pattern="yyyy-MM-dd HH:mm"/>"/>
				</td>
			</tr>
			...
			<tr>
				...
				<td class="h2y_table_edit_td">
					<input name="overflowCount" id="overflowCount" type="text" class="h2y_input_just" disabled="disabled" />
				</td>
				...
				<td class="h2y_table_edit_td">
					<input name="overflowAmount" id="overflowAmount" type="text" class="h2y_input_just" disabled="disabled" />
				</td>
			</tr>
