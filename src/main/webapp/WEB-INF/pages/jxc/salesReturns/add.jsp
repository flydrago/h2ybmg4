...
<head>
    ...
    <script type="text/javascript">
        ...
        $(function () {
        	...
        	$("#returnsDate").datetimepicker({});
        	$("#refundDate").datetimepicker({});
        	//打开 选择客户 对话窗
        	$("#choose_customer_btn").click(function(){
        		openCustomerDialog();
        	});
        	//打开 选择仓库 对话窗
        	...
        	//打开 选择收支账户 对话窗
        	$("#choose_refundAccount_btn").click(function(){
    	  	  ...
    	  	  for(var i=0;i<data.length;i++){
    	  		  ...
    	  		  if(null != data[i].COUNT){
    				  ...
    				  checkedGoodsHtml +=   '<tr id="billGoods_tr_'+data[i].ID+'" class="billGoodsDetail">'+
    		  								...
    		  								'<td class="h2y_table_edit_td"><input name="billGoodsCount" id="billGoodsCount'+data[i].ID+'" type="text" class="h2y_input_just" style="width:60px;"  value="1" onblur="calcBillGoodsTotalAmount('+data[i].ID+')"/></td>'+
    		  								'<td class="h2y_table_edit_td"><input name="singlePrice" id="billGoodsSinglePrice'+data[i].ID+'" type="text" class="h2y_input_just" style="width:60px;"  value="0.00" onblur="calcBillGoodsTotalAmount('+data[i].ID+')"/></td>'+
    		  								'<td class="h2y_table_edit_td"><input name="billGoodsTotalAmount" id="billGoodsTotalAmount'+data[i].ID+'" class="h2y_input_just billGoodsTotalAmount" type="text" class="h2y_input_just" style="width:100px;" value="0.00" disabled="disabled"/></td>'+
    	...
    	//计算应付总额
    	function calcRefundableAmount(){
    		var discount = $("#discount").val();
    		var discountF = discount/100;
    		var refundableAmount = accMul($("#totalAmount").val(),discountF);
    		$("#refundableAmount").val(refundableAmount);
    	}
    	...
    	function h2y_save(){
    		...
    		postData.returnsDate = $("#returnsDate").val();
    		postData.refundDate = $("#refundDate").val();
    		postData.customerId = $("#customerId").val();
    		postData.storageId = $("#storageId").val();
    		postData.brokerId = $("#brokerId").val();
    		postData.refundAccountId = $("#refundAccountId").val();
    		postData.currentRefund = $("#currentRefund").val();
    		...
    		$.post("jxc/salesReturn/save.htm",{postData:JSON.stringify(postData)},function(res){
    			var resJson = eval("("+res+")");
    			if(resJson.resultFlag == 1 || resJson.resultFlag == "1"){
    				alert("单据添加成功。");
    				if (top.f_getframe("jxc_sales_init_htm") != null) {
						top.f_getframe("jxc_sales_init_htm").f_query();
					}
					top.f_delTab("salesReturns_add");
    			}else{
...
<body>
	...
	<form name="editform" method="post" action="" id="editform">
		...
		<table class="h2y_table" style="width:98%;">
			...
			<tr>
				<td class="h2y_table_label_td">
					<input type="button" value="选择客户" class="button" id="choose_customer_btn"/>：
				</td>
				<td class="h2y_table_edit_td">
					<input name="customerId"  id="customerId" type="hidden"/>
					<div id="chosen_customer">
					</div>
				</td>
				
				<td class="h2y_table_label_td">
					<input type="button" value="退货仓库" class="button" id="choose_storage_btn"/>：
				</td>
				<td class="h2y_table_edit_td">
			...
			<tr>
				<td class="h2y_table_label_td">
					<input type="button" value="选择退款账户" class="button" id="choose_refundAccount_btn"/>：
				</td>
				<td class="h2y_table_edit_td">
					<input type="hidden" name="unitAccountId" id="unitAccountId"/>
				...
				<td class="h2y_table_edit_td">
					<input name="currentRefund" id="currentRefund" type="text" class="h2y_input_just"/>
				</td>
			</tr>
			...
			<tr>
				...
				<td class="h2y_table_edit_td">
					<input name="returnsDate" id="returnsDate" class="h2y_input_datetime" type="input" 
						value="<fmt:formatDate value="${bill.returnsDate}"  pattern="yyyy-MM-dd HH:mm"/>"/>
				</td>
				...
				<td class="h2y_table_edit_td">
					<input name="refundDate" id="refundDate" class="h2y_input_datetime" type="input"
						value="<fmt:formatDate value="${bill.refundDate}"  pattern="yyyy-MM-dd HH:mm"/>"/>
				</td>
			</tr>
			...
			<tr>
				...
				<td class="h2y_table_edit_td">
					<input name="refundableAmount" id="refundableAmount" type="text" class="h2y_input_just" disabled="disabled" />
				</td>
