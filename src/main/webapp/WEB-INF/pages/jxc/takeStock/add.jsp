...
<head>
    ...
    <script type="text/javascript">
    	    ...
    	    if(data.length != 0){
    	  	  ...
    	  	  for(var i=0;i<data.length;i++){
    				...
    				checkedGoodsHtml +=   '<tr id="billGoods_tr_'+data[i].ID+'" class="billGoodsDetail">'+
    		  								...
    		  								'<td class="h2y_table_edit_td"><input name="billGoodsInventoryCount" id="billGoodsInventoryCount'+data[i].ID+'" type="text" class="h2y_input_just billGoodsInventoryCount" style="width:60px;"  value="1" disabled="disabled"/></td>'+
    		  								'<td class="h2y_table_edit_td"><input name="billGoodsTakeStockCount" id="billGoodsTakeStockCount'+data[i].ID+'" type="text" class="h2y_input_just billGoodsTakeStockCount" style="width:60px;"  value="1" onblur="calcBillGoodsBreakeven('+data[i].ID+')"/></td>'+
    		  								'<td class="h2y_table_edit_td"><input name="billGoodsBreakevenCount" id="billGoodsBreakevenCount'+data[i].ID+'" type="text" class="h2y_input_just billGoodsBreakevenCount" style="width:60px;"  value="1" disabled="disabled"/></td>'+
    		  								'<td class="h2y_table_edit_td"><input name="costSinglePrice" id="billGoodsCostSinglePrice'+data[i].ID+'" type="text" class="h2y_input_just" style="width:60px;"  value="0.00" onblur="calcBillGoodsBreakeven('+data[i].ID+')"/></td>'+
    		  								'<td class="h2y_table_edit_td"><input name="billGoodsBreakevenAmount" id="billGoodsBreakevenAmount'+data[i].ID+'" class="billGoodsBreakevenAmount" type="text" class="h2y_input_just" style="width:100px;" value="0.00" disabled="disabled"/></td>'+
    	...
    	//计算单件商品 盈亏数量 及 盈亏金额
    	function calcBillGoodsBreakeven(goodsID){
    		var singleGoodBreakevenCount = accMin($("#billGoodsTakeStockCount"+goodsID).val(), $("#billGoodsInventoryCount"+goodsID).val()) ;
    		var singleGoodBreakevenAmount = accMul($("#billGoodsCostSinglePrice"+goodsID).val(), singleGoodBreakevenCount);
    		
    		$("#billGoodsBreakevenCount"+goodsID).val(singleGoodBreakevenCount);
    		$("#billGoodsBreakevenAmount"+goodsID).val(singleGoodBreakevenAmount);
    		
    		BillTakeStock();
    	}
    	
    	//计算单据 库存总数  盘点总数  盈亏总数  盈亏总额
    	function BillTakeStock(){
    		var billInventoryCount = 0;
    		var billTakeStockCount = 0;
    		var billBreakevenCount = 0;
    		var billBreakevenAmount = 0.00;
    		
    		//库存总数
    		$(".billGoodsInventoryCount").each(function(){
    			billInventoryCount = accAdd(billInventoryCount, this.value);
    		});
    		
    		$("#inventoryCount").val(billInventoryCount);
    		
    		//盘点总数
    		$(".billGoodsTakeStockCount").each(function(){
    			billTakeStockCount = accAdd(billTakeStockCount, this.value);
    		});
    		
    		$("#takeStockCount").val(billTakeStockCount);
    		
    		//盈亏总数
    		$(".billGoodsBreakevenCount").each(function(){
    			billBreakevenCount = accAdd(billBreakevenCount, this.value);
    		});
    		
    		$("#breakevenCount").val(billBreakevenCount);
    		
    		//盈亏总额
    		$(".billGoodsBreakevenAmount").each(function(){
				billBreakevenAmount = accAdd(billBreakevenAmount, this.value);    			
    		});
    		
    		$("#breakevenAmount").val(billBreakevenAmount);
    	}
    	...
    	function h2y_save(){
    		...
    		postData.takeStockDate = $("#takeStockDate").val();
    		...
    		//单据 商品明细数组 组装
    			$(".billGoodsDetail").each(function(){
    				...
    				billGoodsSingleDetail.goodsInventoryCount = $(this).find("input[name='billGoodsInventoryCount']").val();
    				billGoodsSingleDetail.goodsTakeStockCount = $(this).find("input[name='billGoodsTakeStockCount']").val();
    				billGoodsSingleDetail.goodsBreakevenCount = $(this).find("input[name='billGoodsBreakevenCount']").val();
    				billGoodsSingleDetail.costSinglePrice = $(this).find("input[name='costSinglePrice']").val();
    				billGoodsSingleDetail.goodsBreakevenAmount = $(this).find("input[name='billGoodsBreakevenAmount']").val();
    		...
    		$.post("jxc/takeStock/save.htm",{postData:JSON.stringify(postData)},function(res){
    			var resJson = eval("("+res+")");
    			if(resJson.resultFlag == 1 || resJson.resultFlag == "1"){
    				alert("单据添加成功。");
    				if (top.f_getframe("jxc_takeStock_init_htm") != null) {
						top.f_getframe("jxc_takeStock_init_htm").f_query();
					}
					top.f_delTab("takeStock_add");
    			}else{
    				/* alert("单据添加失败，请联系管理员"); */
...
<body>
	...
	<form name="editform" method="post" action="" id="editform">
		...
		<table class="h2y_table" style="width:98%;">
			...
			<tr>
				<td class="h2y_table_label_td">
					<input type="button" value="选择盘点仓库" class="button" id="choose_storage_btn"/>：
				</td>
			...
			<tr>
				...
				<td class="h2y_table_edit_td2" colspan="3">
					<input name="takeStockDate" id="takeStockDate" class="h2y_input_datetime" type="input" 
						value="<fmt:formatDate value="${bill.takeStockDate}"  pattern="yyyy-MM-dd HH:mm"/>"/>
				</td>
			</tr>
			...
			<tr>
				...
				<td class="h2y_table_edit_td">
					<input name="inventoryCount" id="inventoryCount" type="text" class="h2y_input_just" disabled="disabled" />
				</td>
				...
				<td class="h2y_table_edit_td">
					<input name="takeStockCount" id="takeStockCount" type="text" class="h2y_input_just" disabled="disabled"/>
				</td>
			</tr>
			...
			<tr>
				...
				<td class="h2y_table_edit_td">
					<input name="breakevenCount" id="breakevenCount" type="text" class="h2y_input_just" disabled="disabled" />
				</td>
				...
				<td class="h2y_table_edit_td">
					<input name="breakevenAmount" id="breakevenAmount" type="text" class="h2y_input_just" disabled="disabled" />
				</td>
			</tr>
