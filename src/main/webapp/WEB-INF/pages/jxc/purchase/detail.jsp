...
<head>
    ...
    <script type="text/javascript">
        $(function () {
        	//$("#toptoolbar").ligerToolBar({items: [{ line:true },{ text: '审核' , click: h2y_audit , icon:'communication' },{text:'红冲' , click:h2y_strikeBalance , icon:'candle' },{ text: '办理过程' , click: h2y_auditLog , icon:'attibutes' },{ text: '刷新' , click: h2y_refresh , icon:'refresh' }]});
		...
		function h2y_audit(){
			...
			var auditBean = "purchaseReceiptsBillAudit";
		...
		//单据红冲
		function h2y_strikeBalance(){
			var checkStatus = $("#billCheckStatus").val();
			var postData = {};
			
			if(checkStatus == "1"){
				if(confirm("确定红冲该单据？")){
					postData.billId = $("#id").val();
					postData.billNo = $("#billNo").val();
					
					$.post("jxc/purchase/billStrikeBalance.htm",{postData:JSON.stringify(postData)},function(result){
						var resJson = eval("("+result+")");
						if(resJson.resultFlag == 1 || resJson.resultFlag == "1"){
							alert("单据已红冲");
							document.location.reload();
						}else{
							alert("单据红冲失败，原因："+resJson.resultMsg);
							document.location.reload();
						}
					});
				}
			}else {
				alert("单据未审核或审核未通过，暂不能红冲！");
			}
		}
	...
	<form name="editform" method="post" action="" id="editform">
		...
		<table class="h2y_table" style="width:98%;">
			...
			<tr>
				...
				<td class="h2y_table_edit_td">
					<div class="billCheckStatus">
							...
							<c:if test="${billDetail.checkStatus eq 9 }">
								<span style="  font-weight: bolder;color: red;">
									已红冲
								</span>
							</c:if>
			...
			<tr>
				...
				<td class="h2y_table_edit_td">
					<input name="supplierId"  id="supplierId" type="hidden" value="${billDetail.supplierId }"/>
					<div id="supplierInfo">
						${billDetail.supplier }
					</div>
				</td>
			...
			<tr>
				...
				<td class="h2y_table_edit_td">
					<input name="paymentAccountId" id="paymentAccountId" type="hidden" value="${billDetail.paymentAccountId }"/>
					<div id="paymentAccountInfo">
						${billDetail.paymentAccount }
					</div>
				</td>
				...
				<td class="h2y_table_edit_td">
					<input name="currentPayment" id="currentPayment" type="text"  class="h2y_input_just" value="${billDetail.currentPayment }" disabled="disabled"/>
				</td>
			</tr>
			...
			<tr>
				...
				<td class="h2y_table_edit_td">
					<input name="receiptsDate" id="receiptsDate" class="h2y_input_datetime" type="input" 
						value="<fmt:formatDate value="${billDetail.receiptsDate}"  pattern="yyyy-MM-dd HH:mm"/>" disabled="disabled"/>
				</td>
				...
				<td class="h2y_table_edit_td">
					<input name="paymentDate" id="paymentDate" class="h2y_input_datetime" type="input"
						value="<fmt:formatDate value="${billDetail.paymentDate}"  pattern="yyyy-MM-dd HH:mm"/>" disabled="disabled"/>
				</td>
			</tr>
			...
			<tr>
				...
				<td class="h2y_table_edit_td">
					<input name="payableAmount" id="payableAmount" type="text" class="h2y_input_just" value="${billDetail.payableAmount }" disabled="disabled" />
				</td>
