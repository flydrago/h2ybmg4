...
<script type="text/javascript">
        ...
        $(function () {
            ...
            if (op == "modify") {
                ...
                h2y_fileModify("android","${goodsTypeLogo.androidFileName}","${goodsTypeLogo.id}");
                h2y_fileModify("ios","${goodsTypeLogo.iosFileName}","${goodsTypeLogo.id}");
                h2y_fileModify("wechat","${goodsTypeLogo.wechatFileName}","${goodsTypeLogo.id}");
        ...
        function h2y_save() {
            ...
            $.post("business/goodstypelogo/save.htm", queryString, function (data) {
                ...
                if (op == "add") {
                    if (jsonReturn.code == "1") {
                        ...
                        if (top.f_getframe("business_goodstypelogo_init_htm") != null) {
							top.f_getframe("business_goodstypelogo_init_htm").f_query();
						}
						top.f_delTab("business_goodstypelogo_add${goodsTypeLogo.typeId}");
                    } else {
                        alert(jsonReturn.msg);
                    }
                } else {
                    if (jsonReturn.code == "1") {
                        ...
                        top.f_delTab("business_goodstypelogo_modify${goodsTypeLogo.id}");
                    } else {
                        alert(jsonReturn.msg);
                    }
                }
        ...
        //修改回显图片
 		function h2y_fileModify(logoType,fileName,id){
        	...
        	var json_str = "{\"id\":\""+id+"\"}";
        	$("#"+logoType+"Logo_div").html("<input type=\"hidden\" name=\""+logoType+"FileData\"  value='"+json_str+"'/><a id=\"file_lightbox_"+logoType+"\" rel=\"lightbox\" href=\"common/image/view.htm?fileBean=goodsTypeLogoService&id="+id+"&type="+logoType+"\"><img class=\""+logoType+"LogoImg\" src=\"common/image/view.htm?fileBean=goodsTypeLogoService&id="+id+"&type="+logoType+"\"></a><a href=\"javascript:deletePic('"+logoType+"');\">删除</a>");
        	$("#file_lightbox_"+logoType).lightBox();
        }
...
<body>
	...
	<form name="editform" method="post" action="" id="editform">
		...
		<input name="id" type="hidden" id="id" value="${goodsTypeLogo.id}" />
		<input name="typeId" type="hidden" id="typeId" value="${goodsTypeLogo.typeId}" />
		<input name="typeCode" type="hidden" id="typeCode" value="${goodsTypeLogo.typeCode}" />
		...
		<table class="h2y_table">
			<tr>
				...
				<td class="h2y_table_edit_td2" colspan="3"> 
					<input name="logoName" id="logoName" class="h2y_input_long" type="input" value="${goodsTypeLogo.logoName}"/>
				</td>
			</tr>
			...
			<tr>
				...
				<td class="h2y_table_edit_td2" colspan="3"> 
					<input name="startDate" id="startDate" class="h2y_input_datetime" type="input" value="<fmt:formatDate value="${goodsTypeLogo.startDate}"  pattern="yyyy-MM-dd HH:mm"/>"/>~
					<input name="endDate" id="endDate" class="h2y_input_datetime" type="input" value="<fmt:formatDate value="${goodsTypeLogo.endDate}"  pattern="yyyy-MM-dd HH:mm"/>"/>
				</td>
			</tr>
			...
			<tr>
				...
				<td class="h2y_table_edit_td">
					<h2y:input name="isDefault" id="isDefault" type="radio" initoption="0,否:1,是" value="${goodsTypeLogo.isDefault}"/>
				</td>
				<td class="h2y_table_label_td">
					状态：
				</td>
				<td class="h2y_table_edit_td">
					<h2y:input name="status" id="status" type="radio" initoption="0,启用:1,不启用" value="${goodsTypeLogo.status}"/>
				</td>
			</tr>
			...
			<tr>
				...
				<td class="h2y_table_edit_td2" colspan="3">
					<input name="memo" id="memo" type="input" value="${goodsTypeLogo.memo}"  class="h2y_input_long"/>
				</td>
			</tr>
