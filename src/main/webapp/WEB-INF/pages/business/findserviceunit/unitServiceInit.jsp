...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		function f_getList() {
	        ...
	        var url_1 = "business/findserviceunit/getUnitServiceList.htm";
		...
		function updateOrd(){
	        ...
	        $.post("business/findserviceunit/saveOrd.htm",{id:rows[0].id,ord:rows[0].ord}, function (data) {
		...
		function h2y_add() {
			...
			if(manager.getData().length>=10){
				...
				openServiceSelectDialog({selectType:"multi"});
			}
		...
		function h2y_delete(){
			...
			<%--注意该处url可能不符合你的要求，请注意修改--%>
			$.post("<%=basePath%>business/findserviceunit/delete.htm?id="+rows[0].id,  function (data){
				
				var jsonReturn=eval("("+data+")");
				if(jsonReturn.code=="1"){	               					
					alert(jsonReturn.msg+"请至少维护4个，建议6、8、10个！");
					f_query();
				}else{
	  ...
	  //打开活动选择列表
	    function openServiceSelectDialog(obj) {
	        ...
	        var src = "business/findserviceunit/unitServiceSelectInit.htm?selectType="+selectType;
	        $.ligerDialog.open({
	            name:"select_service",
	            title:  "选择服务",
	            height: 500,
	            ...
	            buttons: [
	                { text: '确定', onclick: function (item, dialog) {
	                	var data=$("#select_service")[0].contentWindow.h2y_returnData();								  	
	     			  	if (data){
	     			  		h2y_serviceSelectCallBack(data);
	     			  		dialog.close();
	     			  	}
	                } },
	    ...
	    function h2y_serviceSelectCallBack(data){
	        ...
	        <%--注意该处url可能不符合你的要求，请注意修改--%>
	        $.post("business/findserviceunit/saveUnitService.htm?unitId="+${unitId},{serviceIds:JSON.stringify(serviceIds)}, function (data) {
	            var jsonReturn = eval("(" + data + ")");
	            if (jsonReturn.code == "1") {
	                alert(jsonReturn.msg+"请至少维护4个，建议6、8、10个！");
	                f_query();
