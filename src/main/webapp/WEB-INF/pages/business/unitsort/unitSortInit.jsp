...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<script type="text/javascript">
			...
			$(function(){
				try{
					if(null!=parent.unitSortDataJson && undefined!=parent.unitSortDataJson){
						$(parent.unitSortDataJson).each(function(){
							$("#objcolums").append("<option value=\""+this.id+"\" iname=\""+this.text+"\">"+this.text+"</option>");
						})
					}
			...
			function f_query(){    
				...
				var url  = "business/unitsort/getListMapById.htm";
				var params = {id:id};
				//点击二级分类加载到中间框
				if(id != 0){
					$.post(url, params, function (data){
						...
						if(search_data.length>0){
								...
								$(search_data).each(function(){
									$("#oAllsearchcolums").append("<option value=\""+this.id+"\" iname=\""+this.text+"\">"+this.text+"</option>");
								});
							}
			...
			function searchgoods(){
				...
				var params = {searchName:searchName,id:id};
				
				$.post("business/unitsort/getListMapById.htm",params,function (data){
			...
			function  h2y_returnData(){
				...
				$.each($("#objcolums option"),function(i,e){
					...
					if(iflag_ == "p_"){
						optionData = "{id:\""+$(e).val()+"\",text:\""+$(e).attr("iname")+"\"}";
					}else{
						optionData = "{id:\""+$(e).val()+"\",text:\""+$(e).attr("iname")+"\"}";
					}
	...
	<body style="font-size: 12px; font-family: 宋体;">
		<div id="layout1">
			<div position="top" style="padding-top: 2px;text-align: center;">
				 名称:
				<input type=text name='searchName'style="width: 110px; height: 16px;" id='searchName' />
				&nbsp;
