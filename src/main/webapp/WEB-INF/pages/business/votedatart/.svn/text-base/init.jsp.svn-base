...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		function f_getList() {
	        ...
	        var url_1 = "business/sharehrefdatart/getVoteList.htm?hrefId=${hrefId}";
	  ...
	  //打开活动选择列表
	    function openPromoteSelectDialog(obj) {
	        ...
	        var src = "business/sharehrefdatart/voteSelectInit.htm?selectType="+selectType+"&hrefId=${hrefId}";
	        $.ligerDialog.open({
	            name:"select_vote",
	            title:  "选择投票",
	            height: 500,
	            ...
	            buttons: [
	                { text: '确定', onclick: function (item, dialog) {
	                	var data=$("#select_vote")[0].contentWindow.h2y_returnData();								  	
	     			  	if (data){
	    ...
	    function h2y_promoteSelectCallBack(data){
	        ...
	        <%--注意该处url可能不符合你的要求，请注意修改--%>
	        $.post("business/sharehrefdatart/save.htm",{hrefId:${hrefId},str1:'vote',promoteData:JSON.stringify(promoteData)}, function (data) {
	            var jsonReturn = eval("(" + data + ")");
	            if (jsonReturn.code == "1") {
	                alert(jsonReturn.msg);
	                f_query();
