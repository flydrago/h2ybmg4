 ...
 * @author jyd-yfb-02
 */
@Service("purchaseReturnsBillAudit")
public class JxcPurchaseReturnsBillAudit implements IJxcBillAuditService{
	@Autowired
	private IJxcPurchaseReturnsDao jxcPurchaseReturnsDao;
	...
	public Map<String, Object> billAudit(Map<String, Object> reqMap) {
		...
		//获取将要审核的单据
		JxcPurchaseReturns tmpBill = jxcPurchaseReturnsDao.get(billId);
		...
		if(op.equals("audit")){			//审核
			if(checkStatus == 1){	//如果单据审核通过
				...
				transMap.put("storeOp", StorageKeys.exStorage.value());
				...
				//更新单据 审核状态
				jxcPurchaseReturnsDao.update(tmpBill);
