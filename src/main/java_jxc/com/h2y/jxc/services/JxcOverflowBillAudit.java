 ...
 * @author jyd-yfb-02
 */
@Service("overflowBillAudit")
public class JxcOverflowBillAudit implements IJxcBillAuditService{

	@Autowired
	private IJxcStorehouseService jxcStorehouseService;
	
	@Autowired
	private IJxcOverflowDao jxcOverflowDao;
	...
	public Map<String, Object> billAudit(Map<String, Object> reqMap) {
		...
		//获取将要审核的单据
		JxcOverflow tmpBill = jxcOverflowDao.get(billId);
		...
		if(op.equals("audit")){			//审核
			if(checkStatus == 1){	//如果单据审核通过
				...
				transMap.put("storeOp", StorageKeys.overflow.value());
