 ...
 *
 */
@Service("purchaseReturnsBillCheck")
public class PurchaseReturnsBillCheck implements IBillCheckService{
	...
	public JxcBillCheckParams billCheck(Map<String, Object> reqMap){
		...
		//验证通过返回的单据对象
		JxcPurchaseReturns returnsBill = new JxcPurchaseReturns();
		...
		try {
			...
			if(proceedsDate != null && !"".equals(proceedsDate)){
				...
				returnsBill.setProceedsDate(proceedsTime);
			}
		...
		if(supplier == null){
			...
			returnsBill.setSupplierId(supplierIdL);
			returnsBill.setSupplier(supplier.getUnitsName());
		}
		...
		if(proceedsAccountId != null && !"".equals(proceedsAccountId)){
			...
			if(contactsAccount == null){
				...
				returnsBill.setProceedsAccountId(proceedsAccountIdL);
				returnsBill.setProceedsAccount(contactsAccount.getAccountName());
			}
		}
		...
		if(currentProceeds != null && !"".equals(currentProceeds)){
			...
			returnsBill.setCurrentProceeds(currentProceedsD);
		}
		...
		//应收金额
		Double receivableAmount = Arith.mul(billTotalAmount, Arith.div(returnsBill.getDiscount(), 100d));
		returnsBill.setReceivableAmount(receivableAmount);
